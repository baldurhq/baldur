...
- A 429 seen by a `rate_limit_aware` client and the retry around it installs one cooldown, not two.
...
- A crashed scheduler-failover retry thread now says so instead of stranding the host as a follower.
