...
class EntryOperationsMixin:
    ...
    def resolve_entries_batch(
        ...
        Args:
            pks: Entry primary keys to resolve
            resolution_type: How entries were resolved
            status: Target status
            notes: Resolution notes
            chunk_size: Chunk size (default: from DLQSettings)
        ...
        if chunk_size is None:
            try:
                ...
                chunk_size = get_dlq_settings().resolve_batch_chunk_size
            except Exception:
                chunk_size = 500
        ...
        for i in range(0, total, chunk_size):
            ...
            for pk in chunk:
                try:
                    ...
                    failed += 1
                    logger.warning(
                        "dlq.batch_resolve_entry_failed",
                        record_pk=pk,
                        batch_id=batch_id,
                        error=str(e),
                    )
            ...
            logger.info(
                "dlq.batch_resolve_chunk_completed",
                batch_id=batch_id,
                chunk_index=i // chunk_size,
                chunk_attempted=min(chunk_size, total - i),
            )

        logger.info(
            "dlq.batch_resolve_completed",
            batch_id=batch_id,
            total=total,
            resolved=resolved,
            failed=failed,
