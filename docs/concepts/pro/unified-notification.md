...
Unified Notification ships with the PRO tier; the hub is available once PRO is active, and on an OSS-only install every notification call is a silent no-op.
