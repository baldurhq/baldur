...
So that the gate stays cheap on a hot path, each check reads system state through a short-lived cache (a configurable time-to-live, thirty seconds by default). An operator's flip does not wait that cache out: the state changes the gate cares about (the kill switch turning on or off, an emergency level change, an error-budget threshold crossing) each announce themselves as an event that clears the cache on the spot, so the very next check reads live state. That immediacy belongs to the process that sees the event; a process it has not reached yet converges on its own: once its cached answer expires, the check re-reads shared state — the kill-switch answer directly, the emergency and error-budget answers through the consulted subsystem's own equally short cache — so even a flip made on another server is honored within a cache lifetime or two at worst.
