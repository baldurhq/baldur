...
| You got a `Baldur <component> Failure` page (CRITICAL) | The Meta-Watchdog runs detect-and-escalate only by default (no automatic recovery) — a component was unhealthy for ~5 consecutive probe cycles and paged you to intervene | [meta-watchdog-escalation-response.md](meta-watchdog-escalation-response.md) |
...
| A Celery worker stops and you see no `shutdown.worker_exit_completed` line, or DLQ entries captured just before the stop never appear | The stop window is shorter than the shutdown pipeline needs (Kubernetes grants 30 s by default, Baldur needs ~42 s at defaults), or the worker was stopped with SIGQUIT, which reaches no teardown at all | [celery-graceful-shutdown.md](celery-graceful-shutdown.md) |
...
| PagerDuty incidents opened by Baldur **never close on their own**, even after the condition recovered | By design, Baldur sends `resolve` for exactly one incident class — the channel self-test's own synthetic incident, closed in the call that opened it. Component failures, hub alerts, security incidents and daily reports are `trigger`-only and are closed on the PagerDuty side; acknowledge is never sent at all | [notification-channel-topology.md](notification-channel-topology.md#known-limitation--incident-close-is-self-test-only) |
...
| Size a Celery deployment's stop window so a worker stop flushes its DLQ and audit buffers instead of being killed halfway | [celery-graceful-shutdown.md](celery-graceful-shutdown.md) |
