...
| PagerDuty incidents opened by Baldur **never close on their own**, even after the condition recovered | By design at v1.0, Baldur sends `resolve` for exactly one incident class — the channel self-test's own synthetic incident, closed in the call that opened it. Component failures, hub alerts, security incidents and daily reports are `trigger`-only and are closed on the PagerDuty side; acknowledge is never sent at all | [notification-channel-topology.md](notification-channel-topology.md#known-limitation--incident-close-is-self-test-only) |
