...
| [notification-channel-topology.md](notification-channel-topology.md) | Decide which events page PagerDuty vs land in Slack — the shipped severity→channel defaults, the two config homes (`BALDUR_META_WATCHDOG_*` escalation vs `BALDUR_CHANNEL_TARGET_*`/`BALDUR_CHANNEL_ROUTING_*` PRO hub), the opt-in PagerDuty leg (`PAGERDUTY_ENABLED` master switch), the recommended PagerDuty-centric war-room topology (critical→PD only + PD's Slack app for native Ack/Resolve buttons), and the trigger-only limitation (Baldur opens incidents; a human closes them) | 2026-07-21 |
