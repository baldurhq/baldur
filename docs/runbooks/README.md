...
| [runtime-config-change.md](runtime-config-change.md) | Safe procedure for editing runtime config via the console/`/config/*` REST — per-domain blast-radius map, the apply-strategy-vs-"takes effect" reach model (on next read / after worker restart / not currently reached), reading the apply response (Safe-Default clamps, 409), the inline rollback ladder (cancel → history rollback → global reset), and the limitations (6 domains persisted-but-not-consumed, canary/governance advisory) | 2026-08-18 |
