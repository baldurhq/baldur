...
class TestOutboundCooldownLifecycle:
    ...
    def test_429_installs_cooldown_and_next_attempt_defers(self):
        """A real 429 installs a cooldown that the next attempt reads back and defers on.

        The cooldown is NOT pre-seeded — attempt 1's 429 drives on_rate_limited,
        and attempt 2's wait_if_needed reads the just-written value.
        """
    ...
    def test_429_then_fitting_cooldown_is_served_and_call_succeeds(self):
        """A cooldown that fits the budget is waited out, then the retried call succeeds."""
        storage = InMemoryRateLimitStorage()
    ...
    def test_recovery_success_resets_the_consecutive_counter(self):
        """After the retried call succeeds, on_success clears the 429 counter in storage."""
        storage = InMemoryRateLimitStorage()
        coord = _coordinator(storage, default_retry_after=0.5)
        policy = _policy(coord, domain="payment", max_elapsed=30.0)
        calls = {"n": 0}
    ...
    def test_shared_cooldown_defers_a_second_worker_without_its_own_429(self):
        """One worker's 429 cooldown, in shared storage, defers an independent worker.

        Worker B never raises a 429 of its own — it defers purely on the cooldown
        worker A installed, proving the cross-worker shared-state contract.
        """
        storage = InMemoryRateLimitStorage()
